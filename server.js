// ============================================================
// MYG TELECOM - SERVIDOR UNIFICADO v3.6.1
//
// Cambios v3.6.1 sobre v3.6:
// - PATCH /api/rh/movimientos/:id/estado: guarda attuid_creado en MongoDB
// - Helper nodemailer enviarCorreoRH() + htmlTemplateRH()
// - Correo automático al crear movimiento (al solicitante y SISTEMAS/ADMIN)
// - Correo automático al cambiar estado (al creador del movimiento)
//   → Requiere en Render: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS
// ============================================================
...
const COLLECTIONS = {
    ...
    HUB_PETICIONES:         'hub_peticiones',
    ADMIN_ACCESS_LOGS:      'admin_access_logs',
    ROLE_PERMISSIONS:       'role_permissions',
};

// ============================================================
// EMAIL — NODEMAILER v3.6.1
// Variables de entorno requeridas en Render:
//   SMTP_HOST  → ej. cs304.bluehost.com  o smtp.gmail.com
...
function getMailTransporter() {
    ...
    if (!smtpUser || !smtpPass) {
        logger.warn('⚠️ SMTP no configurado (SMTP_USER / SMTP_PASS vacíos). Correos RH desactivados.');
        return null;
    }
    ...
    logger.info(`📧 Nodemailer configurado: ${smtpHost}:${smtpPort} (${smtpUser})`);
    return _mailTransporter;
}

async function enviarCorreoRH(destinatario, asunto, htmlBody) {
    const transporter = getMailTransporter();
    if (!transporter) return { ok: false, reason: 'SMTP no configurado' };
    try {
        const smtpUser = process.env.SMTP_USER || process.env.SMTP_EMAIL;
        await transporter.sendMail({
            from:    `"MYG Telecom - Sistemas" <${smtpUser}>`,
            to:      destinatario,
            subject: asunto,
            html:    htmlBody,
        });
        logger.info(`📧 Correo enviado a ${destinatario}: ${asunto}`);
        return { ok: true };
    } catch (err) {
        logger.error(`❌ Error enviando correo a ${destinatario}:`, err.message);
        return { ok: false, reason: err.message };
    }
}

function htmlTemplateRH(color, titulo, body) {
    return `<!DOCTYPE html>
<html><head><meta charset="UTF-8">
<style>
  body{font-family:Arial,sans-serif;color:#1f2937;margin:0;background:#f9fafb}
  .header{background:${color};color:white;padding:24px 32px;text-align:center}
  .header h1{margin:0;font-size:22px}
  .content{padding:24px 32px;background:#fff;max-width:600px;margin:0 auto}
  .info-box{background:#f3f4f6;border-left:4px solid ${color};padding:12px 16px;margin:16px 0;border-radius:4px}
  .info-box p{margin:4px 0;font-size:14px}
  .footer{background:#374151;color:#9ca3af;padding:12px 32px;text-align:center;font-size:12px}
  strong{color:#1f2937}
</style></head>
<body>
  <div class="header"><h1>${titulo}</h1></div>
  <div class="content">${body}</div>
  <div class="footer">MYG Telecom &copy; ${new Date().getFullYear()} &mdash; Mensaje automático del Sistema RH</div>
</body></html>`;
}

// ============================================================
// CATÁLOGOS, SCHEMAS Y HELPERS
// ============================================================
...
// ============================================================
// TEMPLATE UTILS (formatos de activación)
// ============================================================
    ...
    try {
        ...
        if (!existingPerms) {
            await rolePermsCol.insertOne({
                ...
                actualizadoEn: new Date().toISOString(), actualizadoPor: 'system-v3.6.1',
            });
            ...
            if (toAdd.length > 0) {
                await rolePermsCol.updateOne({ type: 'role_defaults' }, {
                    ...
                    $set: { actualizadoEn: new Date().toISOString(), actualizadoPor: 'system-v3.6.1' }
                });
                logger.info(`✅ Migrados ${toAdd.length} permisos v3.6.1 a SISTEMAS`);
            }
        }
        logger.info('✅ Database indexes created v3.6.1');
    } catch (error) {
...
app.get('/', (req, res) => res.json({ name: 'MYG Telecom - Servidor Unificado', version: '3.6.1', status: 'running' }));
...
app.get('/health', async (req, res) => {
    try {
        ...
        res.json({
            status: 'ok', timestamp: new Date().toISOString(), version: '3.6.1',
            database: { status: 'connected' },
            storage: { templates: plantillasStatus },
...
// ============================================================
// ENDPOINTS — RH MOVIMIENTOS v3.6.1
// ============================================================
...
app.post('/api/rh/movimientos', authMiddleware, async (req, res) => {
    try {
        ...
        // ── Correos de notificación (v3.6.1) ──────────────────────────
        // El error de email NO debe bloquear la respuesta → try/catch independiente
        try {
            const tipoLabel = {
                ALTA: 'Alta', ALTA_UDP: 'Alta UDP', BAJA: 'Baja',
                CAMBIO_PDV: 'Cambio PDV', CAMBIO_PUESTO: 'Cambio Puesto', CAMBIO_COMBINADO: 'Cambio Combinado'
            };
            const nombreEmpleado = `${movimiento.empleado?.nombre || ''} ${movimiento.empleado?.apellido_paterno || ''}`.trim();
            const etiqueta = tipoLabel[movimiento.tipo] || movimiento.tipo;

            // Correo al solicitante (RH)
            if (req.usuario.email) {
                await enviarCorreoRH(
                    req.usuario.email,
                    `✅ Solicitud de ${etiqueta} registrada — ${nombreEmpleado}`,
                    htmlTemplateRH('#3b82f6', `Solicitud de ${etiqueta} Registrada`,
                        `<p>Hola <strong>${req.usuario.nombre}</strong>,</p>
                         <p>Tu solicitud ha sido registrada exitosamente.</p>
                         <div class="info-box">
                           <p><strong>Folio:</strong> ${movimiento._id}</p>
                           <p><strong>Tipo:</strong> ${etiqueta}</p>
                           <p><strong>Colaborador:</strong> ${nombreEmpleado || '—'}</p>
                           <p><strong>Estado:</strong> PENDIENTE</p>
                           <p><strong>Fecha:</strong> ${new Date().toLocaleString('es-MX')}</p>
                         </div>
                         <p style="color:#6b7280;font-size:13px">El equipo de Sistemas procesará tu solicitud a la brevedad.</p>`)
                );
            }

            // Correos a SISTEMAS y ADMIN
            const usersCol = await getCollection(COLLECTIONS.USERS);
            const admins   = await usersCol.find({ rol: { $in: ['SISTEMAS', 'ADMIN'] }, activo: true }).project({ email: 1, nombre: 1 }).toArray();
            for (const adm of admins) {
                if (adm.email && adm.email !== req.usuario.email) {
                    await enviarCorreoRH(
                        adm.email,
                        `🔔 Nueva solicitud de ${etiqueta} — ${nombreEmpleado}`,
                        htmlTemplateRH('#8b5cf6', `Nueva Solicitud: ${etiqueta}`,
                            `<p>Hola <strong>${adm.nombre}</strong>,</p>
                             <p>Se ha registrado una nueva solicitud en el sistema RH.</p>
                             <div class="info-box">
                               <p><strong>Folio:</strong> ${movimiento._id}</p>
                               <p><strong>Tipo:</strong> ${etiqueta}</p>
                               <p><strong>Colaborador:</strong> ${nombreEmpleado || '—'}</p>
                               <p><strong>Solicitante:</strong> ${req.usuario.nombre} (${req.usuario.email || '—'})</p>
                               <p><strong>Fecha:</strong> ${new Date().toLocaleString('es-MX')}</p>
                             </div>
                             <p>Ingresa al dashboard para revisar y procesar la solicitud.</p>`)
                    );
                }
            }
        } catch (emailErr) {
            logger.error('Error enviando correos de notificación RH (POST):', emailErr.message);
        }
...
app.put('/api/rh/movimientos/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const movimiento  = await movimientos.findOne({ _id: new ObjectId(req.params.id) });
        if (!movimiento) return res.status(404).json({ error: 'Movimiento no encontrado' });
        // Solo el creador (si está PENDIENTE) o SISTEMAS/ADMIN pueden editar
        const puedeEditar = ['SISTEMAS','ADMIN'].includes(req.usuario.rol) ||
            (movimiento.estado === 'PENDIENTE' && movimiento.creado_por?.username === req.usuario.username);
        if (!puedeEditar) return res.status(403).json({ error: 'Sin permiso para editar este movimiento' });
        const { _id, creado_por, historial, fecha_creacion, ...updateData } = req.body;
        await movimientos.updateOne(
            { _id: new ObjectId(req.params.id) },
            { $set: { ...updateData, fecha_modificacion: new Date().toISOString() } }
        );
        res.json(await movimientos.findOne({ _id: new ObjectId(req.params.id) }));
    } catch (error) { res.status(500).json({ error: error.message }); }
});

// ── PATCH estado — v3.6.1: guarda attuid_creado + correo automático ──────────
app.patch('/api/rh/movimientos/:id/estado', authMiddleware, requireSistemas, async (req, res) => {
    try {
        const movimientos = await getCollection(COLLECTIONS.RH_MOVIMIENTOS);

        // ← v3.6.1: attuid_creado añadido al destructuring
        const { estado, comentario, correo_creado, attuid_creado, procesado_por } = req.body;
        ...
        if (estado === 'COMPLETADO') {
            ...
            if (attuid_creado)  updates.attuid_creado  = attuid_creado; // ← v3.6.1 NEW
        }
        ...
        const movActualizado = await movimientos.findOne({ _id: new ObjectId(req.params.id) });

        // ── Correo de cambio de estado (v3.6.1) ───────────────────────────
        try {
            const nombreEmpleado = `${movActualizado.empleado?.nombre || ''} ${movActualizado.empleado?.apellido_paterno || ''}`.trim();
            if (movActualizado.creado_por?.email) {
                const colorMap = {
                    COMPLETADO: '#10b981', RECHAZADO: '#ef4444',
                    EN_PROCESO: '#3b82f6', PENDIENTE:  '#f59e0b'
                };
                const color = colorMap[estado] || '#6b7280';
                let extraInfo = '';
                if (estado === 'COMPLETADO' && attuid_creado) extraInfo += `<p><strong>ATTUID asignado:</strong> ${attuid_creado}</p>`;
                if (estado === 'COMPLETADO' && correo_creado) extraInfo += `<p><strong>Correo corporativo:</strong> ${correo_creado}</p>`;

                await enviarCorreoRH(
                    movActualizado.creado_por.email,
                    `🔄 Actualización de solicitud — ${nombreEmpleado}: ${estado}`,
                    htmlTemplateRH(color, `Solicitud Actualizada: ${estado}`,
                        `<p>Tu solicitud ha sido actualizada.</p>
                         <div class="info-box">
                           <p><strong>Folio:</strong> ${req.params.id}</p>
                           <p><strong>Colaborador:</strong> ${nombreEmpleado || '—'}</p>
                           <p><strong>Nuevo Estado:</strong> ${estado}</p>
                           ${extraInfo}
                           ${comentario ? `<p><strong>Comentario:</strong> ${comentario}</p>` : ''}
                           <p><strong>Procesado por:</strong> ${req.usuario.nombre}</p>
                         </div>`)
                );
            }
        } catch (emailErr) {
            logger.error('Error enviando correo de cambio de estado RH:', emailErr.message);
        }

        res.json(movActualizado);
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.post('/api/hub/asistencia', authMiddleware, requireHubAccess, async (req, res) => {
    try {
        ...
        const doc = { _id: new ObjectId(), username: regUsername, nombre: regNombre, fecha, tipo, horaEntrada: horaEntrada || null, horaSalida: horaSalida || null, notas: (notas || '').trim(), registradoPor: req.usuario.username !== regUsername ? req.usuario.username : undefined, creadoEn: new Date().toISOString() };
...
app.post('/api/hub/peticiones', authMiddleware, requirePermisoHub('hub.peticiones.crear'), async (req, res) => {
    try {
        ...
        const doc = { _id: new ObjectId(), username: req.usuario.username, nombre: req.usuario.nombre || req.usuario.username, tipo, fechaInicio, fechaFin: fechaFin || fechaInicio, motivo: motivo.trim(), notas: (notas || '').trim(), status: 'pendiente', revisadoPor: null, revisadoEn: null, comentarioRevisor: null, creadoEn: new Date().toISOString() };
        await col.insertOne(doc);
...
async function startServer() {
    try {
        ...
        app.listen(PORT, () => {
            console.log('\n' + '='.repeat(60));
            console.log('🚀 SERVIDOR UNIFICADO v3.6.1 - MYG TELECOM');
            console.log('='.repeat(60));
            console.log(`✅ Puerto: ${PORT} | Modo: ${IS_CLOUD ? 'CLOUD' : 'LOCAL'}`);
            console.log('\n🔧 Novedades v3.6.1:');
            console.log('   ✅ attuid_creado guardado en MongoDB al completar altas');
            console.log('   ✅ GET /api/rh/movimientos/:id (endpoint individual)');
            console.log('   ✅ PUT /api/rh/movimientos/:id (editar con permisos)');
            console.log('   ✅ Correos nodemailer: al crear y al cambiar estado');
            console.log('   ✅ htmlTemplateRH — template HTML reutilizable');
            console.log('   → Variables requeridas: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS');
            console.log('='.repeat(60) + '\n');
        });
