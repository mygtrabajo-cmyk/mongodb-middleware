...
//                       Almacenado en hub_report_config (_id: report_recipients)
//                       Semilla inicial: n.contreras@mygtelecom.mx, l.oliva@mygtelecom.mx
//           [PREP-RH]   Hub RH preparado en frontend (hub_rh tab, GERENTE_RH/ANALISTA_RH)
//   v5.4.1: [BUG-001] hub_messages → hub_mensajes en /api/hub/activity/summary
//                   [BUG-002] hub_movimientos_rh → rh_movimientos (exec/alerts + kpi/summary)
//                   [BUG-003] headcount → hub_hc_usuarios (kpi/summary + headcount/summary)
//                   [BUG-004] kpi/summary: renombrar campo tickets→solicitudesHub (son vacaciones/permisos)
//                   [BUG-005] headcount/summary: campos AREA/ESTATUS (mayúsculas, schema real)
...
async function enviarResumenSemanal(recipientOverride = null) {
    ...
    try {
        ...
        // Prioridad: override manual → DB → env var → auto-detect
        let destinatarios = (recipientOverride || []).filter(Boolean);

        if (!destinatarios.length) {
            // [FEAT-004b] Primero: destinatarios gestionados desde el dashboard (MongoDB)
            destinatarios = await _getReportRecipients();
        }

        if (!destinatarios.length) {
            // Fallback: Env var (WEEKLY_REPORT_RECIPIENTS=a@b.com,c@d.com)
            destinatarios = (process.env.WEEKLY_REPORT_RECIPIENTS || '')
                .split(',').map(s => s.trim()).filter(Boolean);
        }

        if (!destinatarios.length) {
            // Último recurso: auto-detect ADMIN y GERENTE_OPERACIONES activos con email
...
const AREAS_VALIDAS  = ['Sistemas','Mantenimiento','Credito','Logistica','CoordinacionATT','RH']; // [v5.5] RH añadida para GERENTE_RH / ANALISTA_RH
...
// ── Reporte Semanal — Gestión de Destinatarios ──────────────────
// [FEAT-004b] Destinatarios almacenados en MongoDB (hub_report_config)
// Prioridad: DB → WEEKLY_REPORT_RECIPIENTS env var → auto-detect ADMIN/GERENTE con email

const REPORT_CONFIG_COL = 'hub_report_config';
const REPORT_RECIP_ID   = 'report_recipients';
const REPORT_SEED_EMAILS = ['n.contreras@mygtelecom.mx', 'l.oliva@mygtelecom.mx'];

/**
 * Obtiene destinatarios del reporte desde la DB.
 * Si el documento no existe, lo crea con los emails de seed.
 */
async function _getReportRecipients() {
    try {
        let doc = await db.collection(REPORT_CONFIG_COL).findOne({ _id: REPORT_RECIP_ID });
        if (!doc) {
            // Primera vez: sembrar con los correos predeterminados
            doc = { _id: REPORT_RECIP_ID, emails: REPORT_SEED_EMAILS, updatedAt: new Date(), updatedBy: 'system' };
            await db.collection(REPORT_CONFIG_COL).insertOne(doc);
            console.log('[FEAT-004b] Destinatarios del reporte sembrados:', REPORT_SEED_EMAILS);
        }
        return doc.emails || [];
    } catch (err) {
        console.error('[FEAT-004b] Error leyendo destinatarios:', err.message);
        return [];
    }
}

/** GET /api/admin/weekly-report/recipients — lista destinatarios actuales */
app.get('/api/admin/weekly-report/recipients', requireAuth, requireAdmin, async (req, res) => {
    try {
        const emails = await _getReportRecipients();
        res.json({ ok: true, emails });
    } catch (err) {
        console.error('[FEAT-004b] GET recipients:', err.message);
        res.status(500).json({ ok: false, error: 'Error leyendo destinatarios' });
    }
});

/** POST /api/admin/weekly-report/recipients — añade un email */
app.post('/api/admin/weekly-report/recipients', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        await db.collection(REPORT_CONFIG_COL).updateOne(
            { _id: REPORT_RECIP_ID },
            { $addToSet: { emails: emailLimpio }, $set: { updatedAt: new Date(), updatedBy: req.usuario.username } },
            { upsert: true }
        );
        const doc = await db.collection(REPORT_CONFIG_COL).findOne({ _id: REPORT_RECIP_ID });
        await logAccess(req.usuario.username, 'REPORT_RECIPIENT_ADD', { email: emailLimpio });
        res.json({ ok: true, emails: doc.emails || [] });
    } catch (err) {
        console.error('[FEAT-004b] POST recipients:', err.message);
        res.status(500).json({ ok: false, error: 'Error añadiendo destinatario' });
    }
});

/** DELETE /api/admin/weekly-report/recipients/:email — quita un email */
app.delete('/api/admin/weekly-report/recipients/:email', requireAuth, requireAdmin, async (req, res) => {
    try {
        const emailLimpio = decodeURIComponent(req.params.email).trim().toLowerCase();
        await db.collection(REPORT_CONFIG_COL).updateOne(
            { _id: REPORT_RECIP_ID },
            { $pull: { emails: emailLimpio }, $set: { updatedAt: new Date(), updatedBy: req.usuario.username } }
        );
        const doc = await db.collection(REPORT_CONFIG_COL).findOne({ _id: REPORT_RECIP_ID });
        await logAccess(req.usuario.username, 'REPORT_RECIPIENT_REMOVE', { email: emailLimpio });
        res.json({ ok: true, emails: doc?.emails || [] });
    } catch (err) {
        console.error('[FEAT-004b] DELETE recipients:', err.message);
        res.status(500).json({ ok: false, error: 'Error eliminando destinatario' });
    }
});
...
app.get('/api/exec/alerts', requireAuth, requireEjecutivo, async (req, res) => {
    try {
        ...
        try {
            const n = await db.collection('rh_movimientos').countDocuments({ estado: 'pendiente', createdAt: { $lt: new Date(ahora - 5 * 24 * 60 * 60 * 1000) } });
...
app.get('/api/hub/activity/summary', requireAuth, async (req, res) => {
    try {
        ...
        const [msgs, mtgs, pets, bols] = await Promise.allSettled([
            db.collection('hub_mensajes').aggregate([{ $match: { createdAt: { $gte: hace7d } } }, { $group: { _id: '$area', count: { $sum: 1 }, lastMsg: { $max: '$createdAt' } } }, { $sort: { count: -1 } }]).toArray(),
...
app.get('/api/kpi/summary', requireAuth, async (req, res) => {
    try {
        const ahora = new Date();
        const [solicitudesHub, rhMovsPend, vacPend, agTotal, agOk, hcTotal, usrActivos] = await Promise.allSettled([
            // Solicitudes de hub (vacaciones/permisos) — los tickets de soporte vienen de Google Sheets (frontend)
            db.collection('hub_peticiones').countDocuments({ status: { $in: ['pendiente', 'en_proceso'] } }),
            db.collection('rh_movimientos').countDocuments({ estado: 'pendiente' }),
            ...
            db.collection('hub_hc_usuarios').countDocuments({}),
            db.collection('users').countDocuments({ activo: true }),
        ]);
        const g = (r, d = 0) => r.status === 'fulfilled' ? r.value : d;
        res.set('Cache-Control', 'private, max-age=180');
        res.json({ ok: true, kpis: {
            solicitudesHub: { pendientes: g(solicitudesHub), label: 'Solicitudes hub pendientes (vacaciones/permisos)' },
            rh:             { movimientosPendientes: g(rhMovsPend), vacacionesPendientes: g(vacPend), label: 'Movimientos RH' },
            nebula:         { total: g(agTotal), online: g(agOk), label: 'Agentes Nebula' },
            headcount:      { total: g(hcTotal), label: 'Headcount registrado' },
            usuarios:       { activos: g(usrActivos), label: 'Usuarios del sistema' },
...
// Fuente: hub_hc_usuarios (cargado por upload en headcount-integrado.jsx)
app.get('/api/headcount/summary', requireAuth, async (req, res) => {
    try {
        const [result] = await db.collection('hub_hc_usuarios').aggregate([{ $group: { _id: null, total: { $sum: 1 }, areas: { $push: '$AREA' } } }]).toArray();
        ...
        const [activos, bajas] = await Promise.allSettled([
            db.collection('hub_hc_usuarios').countDocuments({ ESTATUS: { $regex: /activo/i } }),
            db.collection('hub_hc_usuarios').countDocuments({ ESTATUS: { $regex: /baja/i } }),
        ]);
        res.set('Cache-Control', 'private, max-age=300');
