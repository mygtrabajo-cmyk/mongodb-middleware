// ============================================================
// NEBULA AGENT — Rutas para el backend MYG Telecom v1.2.0
//
// Cambios v1.2.0 (sobre v1.1.0):
//   [SRV-1] HIGH_COMMANDS expandido con 6 nuevos comandos HIGH:
//           force_logoff_user, enable_rdp, disable_rdp,
//           set_registry_value, create_scheduled_task, delete_scheduled_task
//   [SRV-2] GET /api/nebula/agent-version — para updater.py (Fase 3)
//   [SRV-3] POST /api/nebula/commands — validación de parámetros requeridos
//   [SRV-4] GET /api/nebula/agents/:machine_id/capabilities
//
// Cambios v1.1.0:
...
async function initIndexes(db) {
    ...
    await idx('nebula_commands', { created_at: 1 }, { expireAfterSeconds: 30 * 24 * 3600, name: 'nebula_cmd_ttl_30d' });
...
// ── [SRV-1] HIGH_COMMANDS v1.2.0 — expandido ─────────────────
// Centralizado aquí para ser reutilizado en POST /api/nebula/commands
const HIGH_COMMANDS = new Set([
    ...
    'deploy_zip',
    // [SRV-1] Nuevos v1.2.0
    'force_logoff_user',     // pérdida de trabajo no guardado
    'enable_rdp',            // expone servicio de red 3389
    'disable_rdp',           // puede interrumpir trabajo remoto
    'set_registry_value',    // modifica comportamiento del sistema
    'create_scheduled_task', // persistencia arbitraria
    'delete_scheduled_task', // puede romper automatizaciones
]);

// ── [SRV-3] Parámetros requeridos por comando ─────────────────
const REQUIRED_PARAMS = {
    ping_host:               ['host'],
    send_message_to_user:    ['message'],
    send_toast_notification: ['message'],
    force_logoff_user:       [],   // username O session_id — validado por el handler del agente
    set_wallpaper:           ['image_path'],
    get_registry_value:      ['path'],
    set_registry_value:      ['path', 'name', 'value'],
...
module.exports = {
    ...
    async init(app, db, ssePush, requireAuth, requireAdmin) {
        ...
        // [ZONE-FIX-2] devuelve config en la respuesta para que el agente la lea
                ...
                if (alerts.length > 0) {
                    for (const alert of alerts) {
                        await db.collection('notificaciones').insertOne({
                            titulo: `⚠️ Nebula: ${hostname}`, mensaje: alert.message, tipo: alert.severity,
                            usuario_destino: '*', creadaPor: 'nebula_agent', leida: false,
                            createdAt: now, meta: { machine_id, alert_type: alert.type }
                        });
        ...
        app.get('/api/nebula/commands/pending/:machine_id', requireAgentAuth, async (req, res) => {
            try {
                ...
                const commands = await db.collection('nebula_commands')
                    .find({ machine_id, status: 'pending' }).sort({ created_at: 1 }).limit(10).toArray();
        ...
        // ═══════════════════════════════════════════════════
        // ZONAS (Authorization: Bearer JWT)
        // ═══════════════════════════════════════════════════

        // [ZON-1] GET /api/nebula/zones
        app.get('/api/nebula/zones', requireAuth, async (req, res) => {
            try {
                const filter = {};
                if (req.query.area) filter.area = req.query.area;
                const zones = await db.collection('nebula_zones').find(filter).sort({ area: 1, name: 1 }).toArray();
                res.json({ zones, total: zones.length });
            } catch (e) { console.error('[Nebula] Error GET zones:', e); res.status(500).json({ error: 'Error obteniendo zonas' }); }
        });

        // [ZON-2] POST /api/nebula/zones
        app.post('/api/nebula/zones', requireAuth, async (req, res) => {
            try {
                ...
                const doc = { name: name.trim(), area: area || 'Sistemas', description: description?.trim() || '', created_by: req.usuario.username, created_at: new Date() };
            ...
            } catch (e) { console.error('[Nebula] Error POST zones:', e); res.status(500).json({ error: 'Error creando zona' }); }
        });

        // [ZON-3] DELETE /api/nebula/zones/:id
        app.delete('/api/nebula/zones/:id', requireAuth, async (req, res) => {
            try {
                ...
                const updated = await db.collection('nebula_agents').updateMany({ zone: zone.name }, { $unset: { zone: '' } });
            ...
            } catch (e) { console.error('[Nebula] Error DELETE zones:', e); res.status(500).json({ error: 'Error eliminando zona' }); }
        });
        ...
        app.patch('/api/nebula/agents/:machine_id/zone', requireAuth, async (req, res) => {
            try {
                ...
                const result = await db.collection('nebula_agents').updateOne({ machine_id }, { $set: updates });
                ...
                res.json({ success: true });
            } catch (e) { console.error('[Nebula] Error PATCH zone:', e); res.status(500).json({ error: 'Error asignando zona' }); }
        });

        // [ZON-5] GET /api/nebula/agents/by-zone
        app.get('/api/nebula/agents/by-zone', requireAuth, async (req, res) => {
            try {
                ...
                const agents = await db.collection('nebula_agents')
                    .find({}, { projection: { _id:1, machine_id:1, hostname:1, ip_address:1, os_system:1, agent_version:1, status:1, last_seen:1, last_stats:1, active_alerts:1, area:1, zone:1, registered_at:1 } })
                    .sort({ area: 1, zone: 1, hostname: 1 }).toArray();
                const normalized = agents.map(a => ({ ...a, status: a.last_seen > TEN_MIN_AGO ? 'online' : 'offline', area: a.area || 'Sin área', zone: a.zone || 'Sin zona' }));
                const grouped = {};
                for (const agent of normalized) {
                    const key = `${agent.area}||${agent.zone}`;
                    if (!grouped[key]) grouped[key] = { area: agent.area, zone: agent.zone, agents: [], online: 0, offline: 0 };
                    grouped[key].agents.push(agent);
                    agent.status === 'online' ? grouped[key].online++ : grouped[key].offline++;
                }
                const groups = Object.values(grouped).sort((a, b) => a.area.localeCompare(b.area) || a.zone.localeCompare(b.zone));
                res.json({ groups, total_agents: normalized.length, total_online: normalized.filter(a => a.status === 'online').length, total_offline: normalized.filter(a => a.status === 'offline').length });
            } catch (e) { console.error('[Nebula] Error agents/by-zone:', e); res.status(500).json({ error: 'Error obteniendo agentes por zona' }); }
        });
        ...
        app.get('/api/nebula/agents/:machine_id', requireAuth, async (req, res) => {
            try {
                ...
                const recentCommands = await db.collection('nebula_commands')
                    .find({ machine_id: req.params.machine_id }).sort({ created_at: -1 }).limit(20).toArray();
                const TEN_MIN_AGO = new Date(Date.now() - 10 * 60 * 1000);
        ...
        // [SRV-4] GET /api/nebula/agents/:machine_id/capabilities
        // Lista todos los comandos disponibles con su nivel de acceso
        app.get('/api/nebula/agents/:machine_id/capabilities', requireAuth, async (req, res) => {
            try {
                const agent = await db.collection('nebula_agents').findOne({ machine_id: req.params.machine_id });
                if (!agent) return res.status(404).json({ error: 'Agente no encontrado' });
                ...
                const capabilities = {
                    LOW: [
                        'get_info','get_processes','get_network_info','get_software_inventory',
                        'get_event_logs','check_windows_updates','get_logged_users','get_open_ports',
                        'get_wifi_profiles','ping_host','get_hardware_info','get_startup_programs',
                        'get_local_users','get_battery_status','get_registry_value','send_toast_notification',
                    ],
                    MEDIUM: [
                        'lock_workstation','kill_process','screenshot','block_usb','unblock_usb',
                        'manage_service','disk_cleanup','clear_temp_files','flush_dns','force_gpupdate',
                        'set_wallpaper','send_message_to_user',
                    ],
                    HIGH: [
                        'reset_password','disable_user','enable_user','enable_bitlocker','install_updates',
                        'run_script','reboot','shutdown','deploy_file','deploy_zip','force_logoff_user',
                        'enable_rdp','disable_rdp','set_registry_value','create_scheduled_task',
                        'delete_scheduled_task',
                    ],
                };

                const total = Object.values(capabilities).flat().length;
                ...
                res.json({
                    ...
                    rate_limits: {
                        reboot: 300, shutdown: 300, force_logoff_user: 60,
                        install_updates: 3600, enable_bitlocker: 600,
                        reset_password: 30, deploy_file: 10, run_script: 5,
                    },
                });
            } catch (e) { console.error('[Nebula] Error capabilities:', e); res.status(500).json({ error: 'Error obteniendo capabilities' }); }
        });

        // [ZONE-FIX-1] GET /api/nebula/agents/:machine_id/config
        // Fallback: el agente lee su config explícitamente desde sync_loop
        app.get('/api/nebula/agents/:machine_id/config', requireAgentAuth, async (req, res) => {
            try {
                const { machine_id } = req.params;
                if (machine_id !== req.agent.machine_id)
                    return res.status(403).json({ error: 'No puedes consultar la configuración de otro agente' });
                const agent = await db.collection('nebula_agents').findOne(
                    { machine_id },
                    { projection: { area: 1, zone: 1, settings: 1, hostname: 1, status: 1 } }
                );
                if (!agent) return res.status(404).json({ error: 'Agente no registrado' });
                console.log(`[Nebula] Config solicitada: ${machine_id.substring(0, 16)}... → area=${agent.area || null} zone=${agent.zone || null}`);
                res.json({ machine_id, area: agent.area || null, zone: agent.zone || null, settings: agent.settings || {}, hostname: agent.hostname || null, config_ts: new Date().toISOString() });
            } catch (e) { console.error('[Nebula] Error GET agent config:', e); res.status(500).json({ error: 'Error obteniendo configuración del agente' }); }
        });
        ...
        app.post('/api/nebula/commands', requireAuth, async (req, res) => {
            try {
                ...
                // [SRV-1] Validación HIGH_COMMANDS v1.2.0
                ...
                // [SRV-3] Validación de parámetros requeridos por comando
                const requiredParams = REQUIRED_PARAMS[command];
                if (requiredParams && requiredParams.length > 0) {
                    const params  = parameters || {};
                    const missing = requiredParams.filter(p => params[p] === undefined || params[p] === null || params[p] === '');
                    if (missing.length > 0)
                        return res.status(400).json({
                            error:    `Parámetros requeridos para '${command}': ${missing.join(', ')}`,
                ...
                const cmdDoc = {
                    machine_id,
                    command,
                    parameters:     { ...(parameters||{}), approved: true, justification: justification||'' },
                    status:         'pending',
        ...
        // GET /api/nebula/commands/:command_id — estado de un comando individual
        // Permite al dashboard hacer polling del resultado sin cargar el historial completo.
        app.get('/api/nebula/commands/:command_id', requireAuth, async (req, res) => {
            try {
                const { command_id } = req.params;
                if (!ObjectId.isValid(command_id))
                    return res.status(400).json({ error: `command_id inválido: "${command_id}"` });
                const cmd = await db.collection('nebula_commands').findOne({ _id: new ObjectId(command_id) });
                if (!cmd) return res.status(404).json({ error: 'Comando no encontrado' });
                res.json(cmd);
            } catch (e) { res.status(500).json({ error: 'Error obteniendo comando' }); }
        });

        // GET /api/nebula/commands — historial con paginación real
        app.get('/api/nebula/commands', requireAuth, async (req, res) => {
            try {
                ...
                const [commands, total] = await Promise.all([
                    db.collection('nebula_commands').find(filter).sort({ created_at: -1 }).skip(skip).limit(limit).toArray(),
                    db.collection('nebula_commands').countDocuments(filter),
                ]);
                res.json({
                    commands,
                    paginacion: { total, page, limit, pages: Math.ceil(total / limit), hasNext: skip + limit < total },
                });
        ...
        app.get('/api/nebula/audit/:machine_id', requireAuth, requireAdmin, async (req, res) => {
            try {
                ...
                const entries = await db.collection('nebula_audit')
                    .find({ machine_id: req.params.machine_id }).sort({ timestamp: -1 }).limit(limit).toArray();
        ...
        app.post('/api/nebula/deploy-file', requireAuth, async (req, res) => {
            try {
                const { machine_ids, filename, data_b64, mime_type, target_path, execute_after, run_as_admin, script_args, justification, sha256_expected } = req.body;
                ...
                const agents   = await db.collection('nebula_agents').find({ machine_id: { $in: machine_ids } }).toArray();
                ...
                for (const agent of agents) {
                    ssePush('*', 'nebula_command_queued', {
                        ...
                        machine_id: agent.machine_id, hostname: agent.hostname, command: 'deploy_file', filename,
                        created_by: req.usuario.username, timestamp: now.toISOString(),
                    });
                }
        ...
        console.log('[Nebula] Rutas v1.2.0 registradas ✅');
        ...
        console.log('[Nebula] Agente:    GET  /api/nebula/agents/:id/config');
        ...
        console.log('[Nebula] [SRV-1]   HIGH_COMMANDS expandido (15 comandos)');
        console.log('[Nebula] [SRV-2]   GET  /api/nebula/agent-version');
        console.log('[Nebula] [SRV-3]   Validación de params requeridos en POST /commands');
        console.log('[Nebula] [SRV-4]   GET  /api/nebula/agents/:id/capabilities');
    },

    // ── Watcher de agentes offline ────────────────────────���
    startOfflineWatcher(db, ssePush) {
        // Marcar agentes sin heartbeat reciente como offline
        setInterval(async () => {
            try {
        ...
        // Marcar comandos atascados en in_progress como timed_out
        // Un comando no debería tardar más de 10 minutos; si lo hace, el agente
        // probablemente murió durante la ejecución y nunca reportará el resultado.
        setInterval(async () => {
            try {
                const stuckThreshold = new Date(Date.now() - 10 * 60 * 1000);
                const result = await db.collection('nebula_commands').updateMany(
                    { status: 'in_progress', dispatched_at: { $lt: stuckThreshold } },
                    { $set: { status: 'timed_out', timed_out_at: new Date() } }
                );
                if (result.modifiedCount > 0) {
                    console.log(`[Nebula] ${result.modifiedCount} comando(s) atascados → timed_out`);
                    ssePush('*', 'nebula_commands_timed_out', {
                        count: result.modifiedCount,
                        timestamp: new Date().toISOString(),
                    });
                }
            } catch (e) { console.error('[Nebula] Error stuck commands watcher:', e.message); }
        }, 5 * 60 * 1000);

        console.log('[Nebula] Offline watcher iniciado (cada 5 min) ✅');
        console.log('[Nebula] Stuck commands watcher iniciado (cada 5 min) ✅');
    },
};
